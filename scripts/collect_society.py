#!/usr/bin/env python3
"""Weekly Society-stream scraper for the Market Intelligence KNOWLEDGE track.
...
(routines/routine_society.md) produces the SAME KNOWLEDGE card atom with
source_type="society" (docs/KNOWLEDGE_CARD_SCHEMA.md, unchanged shared contract).

ONE deliberate addition over Concepts: TRIAGE.

  Concepts is Deep+Data only -> extract everything. Society is Flow-heavy
  (68 sources = 27 Deep + 41 Flow; 66 active + 2 parked). Extracting every Flow
  article would be noisy and burn Opus, so the collector triages at INPUT — it
  decides which articles get WORKLISTED (written to raw/) before the expensive
  extraction step. Triage is deterministic (no LLM), exactly like the rest of the
  collector's filtering. It cuts what is collected/worklisted, NOT what is
  extracted: extraction stays "one source per subagent", unchanged from Concepts.

  Per source TYPE (Society inherits the policy Technology settled on):
    Deep  (type=="Deep")  -> extract-all, recency only, no triage gate (like Concepts).
    Flow  (type=="Flow")  -> RECENCY gate (--days window) + PER-SOURCE CAP
            (keep at most N_FLOW newest per run). The WATCHLIST gate is OFF for
            Society (WATCHLIST_GATE=False) — perishable/news Flow is PARKED
            (collect:false, skip_reason "-> FLOW/Newsletters") rather than gated,
            so the active Flow set is durable knowledge, capped on volume only.
    FIREHOSE (Flow sources the curation flagged high-volume/aggregator in
            config _triage.flagged_high_volume_or_aggregator) -> strictest cap
            N_FIREHOSE (< N_FLOW). Society's seed flags NONE (no _triage block),
            so this tier is currently empty; the mechanism is retained from the
            Technology clone for parity.

  The watchlist machinery lives in config/society_watchlist.json (OWNER-CURATED
  flat term list) but is UNUSED while WATCHLIST_GATE=False. If the gate is ever
  re-armed and the file is empty/missing, the gate stays DISABLED (pass-through)
  with a loud warning, so the collector still runs while the owner curates it.
...
Per-run diagnostics (funnel + manifest) land in
    raw/society/_runs/<YYYY-MM-DD>/{funnel.txt,funnel.json,run.json}
...
Run:
    python3 scripts/collect_society.py                       # all sources, --days 7
    python3 scripts/collect_society.py --ids aeon,the_diplomat,council_on_foreign_relations
    python3 scripts/collect_society.py --days 7 --budget 180
"""
...
CONFIG_PATH = REPO_ROOT / "config" / "society_sources.json"
WATCHLIST_PATH = REPO_ROOT / "config" / "society_watchlist.json"
RAW_ROOT = REPO_ROOT / "raw" / "society"
RUNS_ROOT = RAW_ROOT / "_runs"

# --- Tunables ---------------------------------------------------------------
LOOKBACK_DAYS = 7             # weekly stream: only keep articles within this window
...
# Watchlist relevance gate. After the wide-diagnostic re-cut, Society runs over
...
def load_watchlist(path=WATCHLIST_PATH):
    ...
    The file is config/society_watchlist.json with a flat `terms` list (string
...
# the real article — confirmed on Foreign Affairs
# (raw/society/foreign_affairs/da11902084ae95ef.json): a 413-word teaser
# silently overwrote a good 2698-word raw on 2026-07-05, orphaning 10 verified
# quotes on an already-published card.
#
# Two signals combined: a known paywall-marker PHRASE is present, AND the
# total text is short. The length gate is load-bearing, not cosmetic — the
# SAME Foreign Affairs full article (all 2698 words of it) carries the exact
# same boilerplate subscribe footer ("Unlock access to the Foreign Affairs
# app... Already a subscriber? Sign In") as the teaser does, because that CTA
# is site chrome on every FA page regardless of whether the article itself is
# paywalled. Marker presence alone is not enough to tell a teaser from a
# complete article on a site like this; word count is what actually
# distinguishes "a 400-word preview with a subscribe wall" from "a 2700-word
# article with a subscribe ad at the bottom."
TEASER_MARKER_RE = re.compile(
    ...
    r"already a subscriber\??\s*(sign in|log in)|"
...
TEASER_MAX_WORDS = 700  # texts this short (or shorter) that also carry a
                        ...
                        # a real long-form piece; the FA teaser was 413 words,
                        # the FA full article was 2698.
...
def looks_like_teaser(text):
    ...
    than TEASER_MAX_WORDS. See the module comment above for why length (not
    marker presence alone) is the load-bearing signal."""
    if not text:
        return False
...
def collect_source(page, limiter, source, cutoff_date, errors, collected_at,
    ...
    raw/society/<source_slug>/<url_hash>.json."""
    ...
    # Society sources carry a single index_url; wrap it into the URL list the
    # crawl logic expects.
    ...
    for article_url in candidates:
        ...
        # (a) WATCHLIST gate — DISABLED for Society (durable extract-all set).
...
# length to be allowed to overwrite it. Confirmed cause of the Foreign Affairs
# incident: a 413-word paywall re-fetch (15% of the prior 2698 words) silently
# replaced a good raw with no guard at all.
...
def write_source_records(records, funnels=None):
    ...
    record_ids (no existing file) always write, exactly as before."""
    new = 0
    for rec in records:
...
def main():
    ...
    if not WATCHLIST_GATE:
        ...
        log("WARN: watchlist empty/missing — Flow watchlist gate DISABLED "
            "(pass-through). Curate config/society_watchlist.json.")
    else:
    ...
    log(f"Collecting Society stream from {len(sources)} source(s); "
