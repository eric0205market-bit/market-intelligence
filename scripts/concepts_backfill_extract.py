#!/usr/bin/env python3
"""Historical (backfill) Concepts insight extraction — JSON-only, NO publish.
...
quote_verified postprocess) — differing ONLY in I/O paths and the publish step:
  ...
  * quarantines topic-mismatch cards concepts-history/_quarantine/<slug>/<record_id>.json
  * writes a run summary to          concepts-history/_runlog/extract_<YYYY-MM-DD>.md
  * NEVER renders HTML, rebuilds the dashboard, or touches git.

The quality logic is REUSED (imported) from publish_concepts.py — not forked:
  postprocess_record() (sets quote_verified by checking the quote against the raw
  text), entity_presence(), and ENTITY_PRESENCE_MIN. The extraction prompt is the
  daily routine_concepts.md STEP 2 rubric (single-sourced from the routine file) +
  the raw record, so quality is identical to the daily path.

EXCLUDES the heavy think-tank slugs (brookings_institution, carnegie_endowment) —
pending the Society/Science taxonomy decision; they are NOT extracted here.

Subcommands (identical contract to youtube_backfill.py):
...
import publish_concepts as pc  # noqa: E402  -> postprocess_record, entity_presence, ENTITY_PRESENCE_MIN

REPO = Path(__file__).resolve().parent.parent
ROUTINE = REPO / "routines" / "routine_concepts.md"

# concepts-history/ lives OUTSIDE the git repo (sibling of youtube-history/). Use the
# same literal the collector (backfill_concepts_history.py) writes to; override via
# $CONCEPTS_HISTORY_ROOT or --history-root. (Literal avoids importing the collector,
# which would pull in Playwright.)
DEFAULT_HISTORY_ROOT = (
    Path.home() / "Dropbox (Personal)" / "Business" / "InvestTool"
    / "market-intelligence" / "concepts-history"
)
HIST_DIR = Path(os.environ.get("CONCEPTS_HISTORY_ROOT") or DEFAULT_HISTORY_ROOT).expanduser()
PROC_DIR = HIST_DIR / "_processed"
QUAR_DIR = HIST_DIR / "_quarantine"
RUNLOG_DIR = HIST_DIR / "_runlog"

# Reserved top-level names under concepts-history/ (outputs/state, not source slugs).
...
def _hist_records():
    """All backfill article records: concepts-history/<slug>/<record_id>.json,
...
# NOTE vs youtube_backfill: the daily Concepts rubric tells the agent to WRITE
...
# --- prompt (daily rubric, single-sourced from routine_concepts.md) ---------
def _rubric():
    """STEP 2 rubric text from routine_concepts.md (PROCESS..just before STEP 3),
    with the repo-path DELIVERY subsection stripped (backfill overrides delivery)."""
    try:
        txt = ROUTINE.read_text(encoding="utf-8")
    except OSError:
        return "(routine_concepts.md not found — extract per docs/KNOWLEDGE_CARD_SCHEMA.md)"
...
def cmd_prompt(args):
    ...
    # per the DELIVERY section of build_prompt(). So refusing to EMIT the prompt is
    # the only code-enforced protection point against an agent silently overwriting
    # a finished card (a stale wave list, an overlapping wave, or a retried agent
    # whose original later completes would otherwise all be able to clobber it).
    # concepts-history/ is outside git, so an overwrite there is unrecoverable —
    # fail closed by default.
    status = _card_status(args.slug, args.record_id)
    if status in ("finalized", "quarantined") and not getattr(args, "force", False):
        sys.exit(f"REFUSING: concepts-history/{'_processed' if status == 'finalized' else '_quarantine'}"
    ...
    if status in ("finalized", "quarantined"):
        _err(f"WARNING: --force overriding {status} status for {args.slug}/{args.record_id} "
             f"— this agent's write can overwrite that card. concepts-history/ is outside git; "
             f"this cannot be undone.")
    d = _hist_record(args.slug, args.record_id)
    if not d:
        sys.exit(f"history record not found: concepts-history/{args.slug}/{args.record_id}.json")
...
def _finalize_one(slug, rid):
    ...
    quotes, verified = pc.postprocess_record(card, raw)

    # 2) entity-presence guard — reused threshold/function from the daily path.
    frac, n_ent = pc.entity_presence(card, raw)
    if frac < pc.ENTITY_PRESENCE_MIN:
...
def _write_summary(date, processed, quarantined, label="finalize"):
    ...
    lines = [f"## Backfill Concepts {kind} run — {ts}", ""]
    lines.append(f"- Articles {'healed' if label == 'heal' else 'processed'} (kept): **{len(processed)}**")
    lines.append(f"- Quarantined (entity-presence < {int(pc.ENTITY_PRESENCE_MIN*100)}%): **{len(quarantined)}**")
...
def main():
    p = argparse.ArgumentParser(description="Historical (backfill) Concepts extraction — JSON-only, no publish.")
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--history-root", default=None,
                        help="override concepts-history/ root (default: same as the collector)")
