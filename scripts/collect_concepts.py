...
# (raw/society/foreign_affairs/da11902084ae95ef.json, Society): a 413-word
# teaser silently overwrote a good 2698-word raw on re-collection, orphaning
# 10 verified quotes on an already-published card.
#
...
# SAME Foreign Affairs full 2698-word article carries the identical
# boilerplate subscribe footer as its own teaser does, because that CTA is
# site chrome on every FA page regardless of whether the article itself is
...
# distinguishes "a 400-word preview with a subscribe wall" from "a
# 2700-word article with a subscribe ad at the bottom." Both required.
...
def write_source_records(records, funnels=None):
    ...
    the count of NEW files written. Called right after each source so an
    interrupted run keeps everything collected so far instead of losing it
    all. This is the SINGLE write path shared by every collection route
    (plain Playwright, fetch_mode:"rss", and --residential — residential only
    changes which sources are allowed to run via skip_reason_for(), it does
    not bypass this function), so the guard below covers all of them.
