#!/usr/bin/env python3
"""Society-stream extraction helper + publisher (KNOWLEDGE track).
...
routine in routines/routine_society.md, ONE source per subagent. This script
provides the surrounding deterministic machinery:

  worklist                 list new raw/society records with no processed card
                           yet (dedup against processed/society/<record_id>.json)
  postprocess <id...>      set quote_verified on each insight (quote present in
                           the raw article text). Society has no timestamps.
  publish [--date] [--ids] entity-presence guard -> quarantine topic-mismatch
                           cards (NOT deleted), render NEW-ONLY
                           reports/society_<date>.html, rebuild the dashboard.

Card contract: docs/KNOWLEDGE_CARD_SCHEMA.md (v1). source_type="society".

Raw layout : raw/society/<source_slug>/<record_id>.json   (collector output)
Processed   : processed/society/<record_id>.json           (one card per source)
Report      : reports/society_<YYYY-MM-DD>.html            (NEW-ONLY, this run)
Quarantine  : processed/society/_quarantine.json           (non-destructive)
...
RAW_ROOT = REPO / "raw" / "society"
PROC_DIR = REPO / "processed" / "society"
TEMPLATE = REPO / "templates" / "society_report.html"
...
def cmd_worklist(_args):
    ...
    if not rows:
        print("No new Society articles to extract.")
        return
