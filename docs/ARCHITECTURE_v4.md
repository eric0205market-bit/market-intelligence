...
## Revised: Aug 7, 2026 — Concepts fully COMPLETE (daily + history), backfill agents booted out (Technology too). RULE 8 upgraded to three layers (collector path-exclude + worklist filter w/ cookie-consent position-gate + deterministic purge, one shared drop-list); 4th junk shape (consent-only capture) recorded. IEA residential DECLINED. Three-layer junk fix flagged to propagate to Technology/Society. Speaker rubric fix DEFERRED — wrong field, not fabrication; computable at load; no longer a base-load blocker (near-dup/chrome provenance gate UNCHANGED as a hard precondition); 4c stays DECLINED.
## Revised: Aug 9, 2026 — Routine progress-commits made local-only (Pages storm root-fixed); YouTube extraction guard-gap found & corrected (corpus benign per Jul-10 audit); routine-sync process risk recorded; PDF clarified as links-only-by-design (not a gap); Pages deploy verification deferred.
...
- **Chart vision** (chart_data population via Sonnet) is a precondition of a complete base load. Charts currently accumulate as URLs unprocessed (tier-routing off). A base loaded without it leaves the visual-heavy streams (Twitter Banks, Institutional) largely unextracted.
- **PDF handling — LINKS-ONLY by design, NOT a gap.** `collect_research_pdfs.py` deliberately surfaces links to already-public research PDFs (no download, no extraction, no summarization — copyright/paywall reasons; "the reader opens the PDFs himself"). PDFs are a curated link catalog for personal reading, NOT extraction fodder. There is nothing to "ingest" into the base as content. Optional future product choice: load the link CATALOG (bank, title, date, url) as metadata — trivial, not extraction. Do not design PDF content extraction: it would break the intentional links-only decision.
- ⚠ **PROVENANCE CHECK (track-wide blocker, discovered by the Society backfill Jul-10).** quote_verified + entity-guard verify PRESENCE, not ORIGIN. A quote can be verbatim and an entity truly present while BOTH come from template chrome (sidebars: "The Latest / Related / Most Popular", nav rails), not the article body. Society backfill passed 22 fabricated the_point cards + 15 "Archives" listings (one card = ~10 unrelated teasers) GREEN on both guards; only a corpus-wide near-dup scan caught them, post-hoc. Any source with a related/latest rail is an injection vector. The rubric + guard are SHARED across all streams → YouTube/Concepts/Technology may carry the same contamination. FIX: add a provenance check to the rubric (claim/quote text must trace to the article body, isolated from detected chrome zones) AND run it over already-extracted corpora BEFORE load, not after. **Audit results (Jul-10), 3 streams:** YouTube 0/2508 (chrome vector structurally N/A — transcript, not scraped DOM; scoped quote_verified gives partial provenance Society lacked). Technology 0/3603 confirmed (near-dup clusters all benign: UTM-dup URLs, legit cross-publication, source boilerplate). Concepts 12/2608 (0.5%, all anthropic_blog Related-sidebar bleed) — FIXED surgically. Society was the outlier (22+15). → Chrome vector largely a non-issue except Society; three corpora chrome-clear for load.
- ⚠ **SPEAKER PROVENANCE — DEFERRED (owner, Aug-07): NO LONGER A BASE-LOAD BLOCKER.** A speaker mismatch is a wrong FIELD, not fabrication — insight / quote / source are correct — and it is deterministically computable at load time (flag `speaker != raw author`). A rubric fix only improves FUTURE cards; existing cards are corrected by a load-time pass either way. It blocks the base ONLY if speaker-filtered queries are required. Concepts (128) and Society (142) are already fixed; Technology (~550–900) stays flagged-not-fixed. No rubric edit now. **This deferral covers speaker ONLY — the near-dup / chrome PROVENANCE gate below remains a HARD precondition** (fabricated insights are a different failure class). Original finding and the three-part contract, retained for whenever this is picked up: discovered by the Society backfill Jul-10; speaker is a contract field of schema v1. Extraction subagents systematically replaced raw `author` with a name from the article body, "inferring the convention" from neighboring cards (self-reinforcing). Society: 142 cards / 962 insights across 11 sources, fixed. **Audit results (Jul-10): systemic across all scraping streams, but MUST be split, not bulk-reverted.** Concepts 450 mismatched cards split deterministically: 322 = raw author was generic/CMS/empty and the card used the real byline from the text → MORE accurate, KEPT; 128 (1100 insights) = a person QUOTED IN THE BODY used as speaker → genuine misattribution, FIXED to raw author. Bulk-reverting all 450 would have corrupted the 322. Technology ~20% (est. 550–900/3603) same pattern (W&B 6/6: raw author always generic, body author substituted). Society 142/962 (fixed). YouTube 0 (role-inference legit per rubric). → The rubric speaker contract must be THREE-part: (1) raw author normal → speaker=raw author; (2) raw author generic/handle/empty → real byline from a standalone byline-line in the text; (3) a person merely QUOTED in the body → NEVER speaker. A naive "speaker = raw author always" fix REGRESSES the 322-class.
...
- **MANDATORY pre-DB near-dup/chrome gate (build once, run per-stream).** The corpus-wide near-dup scan is the ONLY thing that caught real contamination (presence-guards + manual spot-checks did not). Cheap (pure text, no LLM, seconds/thousands), track-agnostic. Two passes: (1) near-dup — flag insight/quote strings recurring across DIFFERENT-title cards (chrome/listing tell); (2) speaker-vs-byline — flag speaker not equal to raw author AND not a standalone byline-line (genuine misattribution), separated from benign byline-corrections. Gate every stream's Supabase load on it. Society caught its version only post-hoc.
- **Model correlation (Concepts):** contamination skews to Opus, not Sonnet (chrome cards 100% Opus; near-dup touch 10.6% Opus vs 5.1% Sonnet) — the mid-corpus Opus→Sonnet switch did NOT worsen the body/sidebar boundary.
...
**RULE 8 — Junk needs THREE layers, not just a worklist filter.** A worklist junk filter alone treats the symptom: the collector keeps re-scraping the same known-junk URLs every run, so purge has permanent recurring work. Full fix: (1) **collector-side path exclude** — known-junk URLs (team-bio, legal/compliance, careers, cookie/stock widgets, glyph-render pages) are dropped in Pass 1 BEFORE fetch/write, never enter `raw/`; (2) **structural worklist filter** as a safety net — word-floor + bot-page / glyph / archive-listing / **cookie-consent** signals; SKIP not delete; (3) **deterministic purge** for anything already orphaned, wired to run repeatably (not hand `git rm`). ONE shared drop-path list across collector + worklist (Concepts: `WORKLIST_DROP_PATHS` in `publish_concepts.py`, imported by `collect_concepts.py`), not two lists. ⚠ **Fourth junk shape — cookie-consent-only capture:** scraper caught only the consent overlay; body is plain-English consent copy with NO CAPTCHA/glyph/archive tell. The reliable signal is POSITION, not density: real pages have substantial content BEFORE the first consent phrase; junk opens with it at word 0. Naive consent-word density false-positived on real IISS pages (3–3.6%). This extends RULE 2: position-gate, not density-gate. Concepts: 0 false positives across 757 kept cards. Technology/Society daily worklists currently have only the symptom filter — they need all three layers.

*Underlying mechanics (why layer 2 alone can never finish the job):* `record_id` = URL hash, so a record collected BEFORE a source is fixed carries a URL family the new collect path will NEVER overwrite — no collision ever occurs. Such records are not "awaiting overwrite"; they are ORPHANED and resurface in the extraction worklist FOREVER, blocking the run. (Concepts daily 2026-07-10: 15 worklisted, 15 junk, 0 cards — a whole day's collection lost.) Root cause of the first miss: Concepts' BACKFILL worklist had a structural filter (`_worklist_drop`: word-floor + junk-path); the DAILY worklist had NONE — the filter was cloned to the wrong place. Junk-skips MUST also be visible in the health check, never silent — otherwise a source quietly emitting only junk (Damodaran did for ~3 weeks) blocks the routine every morning unnoticed.
