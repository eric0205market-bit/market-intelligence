...
- **Residential-collection path for cloud-IP-blocked sources — reusable path now exists.** Some sources collect fine from a residential IP but the GitHub Actions IP range is blocked (~2 anchors). A reusable `--residential` gate + `residential_only:true` is available; each stream ports the gate into its own collector — this **SUPERSEDES the old "shared-supplement only / do NOT build a per-stream residential path" guidance.** **Chatham House — RESOLVED via RSS** (fetch_mode:rss, `path/83/feed.xml`, full-body items, works from the cloud cron on any IP; no residential path needed — removed from the residential list). Of the original cohort only **IEA Oil Market Report** was left needing the residential path — and it is **DECLINED by the owner (2026-08-07)**: keeping the Mac awake for a single source is not worth it. IEA is simply not collected; the residential machinery stays documented for future use. Not 'pending', not 'residential-recoverable'. YouTube already collects residentially. (Technology/Society may hit more.)
...
- **Automated purge path for orphaned/immortal junk raw (RULE 8) — RESOLVED for Concepts (2026-08-07), still OPEN as a port target for Technology/Society.** `record_id`=URL-hash records the new collect path never overwrites, so they resurface in the worklist forever (a worklist filter SKIPS them but does not remove them, and the collector keeps re-scraping the same URLs every run). **Concepts closed it at both ends: a deterministic repeatable `worklist --purge` step (no hand `git rm`) + a collector-side path exclude that drops known-junk URLs in Pass 1 before fetch/write, off one shared `WORKLIST_DROP_PATHS` list.** Technology/Society still carry only the symptom filter → port all three layers (see ARCHITECTURE RULE 8).
