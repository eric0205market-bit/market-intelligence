# ROUTINE — CONCEPTS EXTRACTION (KNOWLEDGE track)
# Model: Opus (subscription phase — all articles on Opus). NEVER fall back to the paid API.
# Input: raw/concepts/<source_slug>/<record_id>.json  (articles collected by scripts/collect_concepts.py)
# Output: processed/concepts/<record_id>.json  + reports/concepts_<date>.html, published via scripts/publish_concepts.py
...
This routine is the Concepts analog of routine_youtube.md, adapted for TEXT
articles instead of transcripts. The insight atom is IDENTICAL across the
KNOWLEDGE track (YouTube / Concepts / Technology / Society) so everything lands
in Supabase as one clean import — see docs/KNOWLEDGE_CARD_SCHEMA.md.

**NO FLOW SOURCES — extract ALL.** Concepts has no high-volume "flow" tier, so
there is NO triage step: every worklisted article is extracted. (The Technology
clone of this routine ADDS triage there, because Technology has flow sources.
Do not add triage here.)

## STEP 0 — WORKLIST GATE (MANDATORY, RUN FIRST)
Before anything else, build the worklist — every raw article that has NO
`processed/concepts/<record_id>.json` yet (record_id-dedup against processed):

    python3 scripts/publish_concepts.py worklist

If it prints "No new Concepts articles to extract.": STOP. Generate nothing,
write no JSON/HTML, commit/push nothing. Output that one line and end the run.
Continue below ONLY if the worklist has >= 1 article. (This is the Concepts
...
copy (no git clone — this routine runs in the already-checked-out dedicated clone):
    ...
    raw/concepts/<source_slug>/<record_id>.json
...
- claim: must stand alone WITHOUT the title for context. Bad: "He's bullish."
  Good: "Dan Loeb is adding AI-infra credit because spreads don't price obsolescence risk."
...
- speaker: the article's author (raw `author`). If the raw `author` is empty, use
  the `source_name` (e.g. "a16z", "BIS"). Do NOT guess a person's name.
...
Stamp processed_at with the current UTC time. Set source_type="concepts".

### DELIVERY
Write the JSON object to `processed/concepts/<record_id>.json` (valid, parseable,
...
the NEW-ONLY `reports/concepts_<date>.html` (only this run's cards), rebuilds the
dashboard (KNOWLEDGE → Concepts), and then you commit & push to `main`. Run exactly
...
auto-deleted** — logged to `processed/concepts/_quarantine.json`, and printed in the
...
python3 scripts/publish_concepts.py postprocess ${IDS//,/ }
python3 scripts/publish_concepts.py publish --date "$(date -u +%F)" --ids "$IDS"
git add processed/concepts reports/concepts_*.html index.html
git commit -m "concepts insights: +N article(s) ($(date -u +%F))"
for i in 1 2 3; do git push origin main && break; git pull --rebase origin main; done
```

Do NOT render or PUSH by hand beyond these commands — `publish_concepts.py
...
- LOCAL ONLY: `git add processed/concepts/ && git commit -m 'concepts insights: progress (local)'`
- NEVER push these. NEVER run `publish_concepts.py` for them.
