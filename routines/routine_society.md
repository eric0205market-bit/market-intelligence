# ROUTINE — SOCIETY EXTRACTION (KNOWLEDGE track)
# Model: Opus (subscription phase — all articles on Opus). NEVER fall back to the paid API.
# Input: raw/society/<source_slug>/<record_id>.json  (articles collected by scripts/collect_society.py)
# Output: processed/society/<record_id>.json  + reports/society_<date>.html, published via scripts/publish_society.py
# Card contract: docs/KNOWLEDGE_CARD_SCHEMA.md (v1) — the SHARED envelope+theme+insight atom for the whole KNOWLEDGE track.
# Cadence: WEEKLY. On demand, after the weekly collection, run on subscription when quota is fresh. Runs LOCALLY in the dedicated clone.

This routine is the Society analog of routine_concepts.md, for TEXT articles.
...
**TRIAGE HAPPENS AT COLLECTION — NOT HERE.** Society is Flow-heavy (68 sources
= 27 Deep + 41 Flow; 66 active + 2 parked). The deterministic triage that decides
which Flow articles are worth extracting lives in the COLLECTOR
(`scripts/collect_society.py`): Deep sources extract-all; Flow sources are
recency-gated + per-source capped (N=25 newest/run) BEFORE anything is written to
`raw/society/`. The WATCHLIST gate is OFF (WATCHLIST_GATE=False, durable
extract-all) — perishable/news Flow is PARKED to Newsletters at the config level
(collect:false), not gated here. So by the time you see a worklisted article, it
has already passed triage. Do NOT add a triage step here — extract every
worklisted article, exactly like Concepts. (One source per subagent, unchanged.)

## STEP 0 — WORKLIST GATE (MANDATORY, RUN FIRST)
Before anything else, build the worklist — every raw article that has NO
`processed/society/<record_id>.json` yet (record_id-dedup against processed):

    python3 scripts/publish_society.py worklist

If it prints "No new Society articles to extract.": STOP. Generate nothing,
write no JSON/HTML, commit/push nothing. Output that one line and end the run.
Continue below ONLY if the worklist has >= 1 article. (This is the Society
...
copy (no git clone — this routine runs in the already-checked-out dedicated clone):
    ...
    raw/society/<source_slug>/<record_id>.json
...
Stamp processed_at with the current UTC time. Set source_type="society".

### DELIVERY
Write the JSON object to `processed/society/<record_id>.json` (valid, parseable,
...
the NEW-ONLY `reports/society_<date>.html` (only this run's cards), rebuilds the
dashboard (KNOWLEDGE → Society), and then you commit & push to `main`. Run exactly
...
auto-deleted** — logged to `processed/society/_quarantine.json`, and printed in the
...
python3 scripts/publish_society.py postprocess ${IDS//,/ }
python3 scripts/publish_society.py publish --date "$(date -u +%F)" --ids "$IDS"
git add processed/society reports/society_*.html index.html
git commit -m "society insights: +N article(s) ($(date -u +%F))"
